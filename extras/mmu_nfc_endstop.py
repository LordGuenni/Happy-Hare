...
# ── Happy Hare compatibility helpers (inlined) ──────────────────────────────

def _create_mmu_runout_helper(printer, name):
...
def _register_nfc_endstop(mmu, nfc_gate, endstop):
...
class MmuNfcEndstop:
    def __init__(self, config):
        ...
        if self._register_sensor:
            self.runout_helper = _create_mmu_runout_helper(
    ...
    def _handle_connect(self):
        ...
        try:
            rail_description = _register_nfc_endstop(mmu, nfc_gate, self)
